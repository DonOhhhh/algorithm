...
public class training1 {
    ...
    public static int[] countApart(int[][] matrix)
    {
        ...
        for(int i = 0 ; i < matrix.length ; i++)
        {
            ...
            {
                if(matrix[i][j]==1)
                    linkedMap.put(new int[]{i,j},isBoundary(new int[][]{{i,j+1},{i,j-1},{i+1,j},{i-1,j}}, matrix.length, matrix.length));
            }
        }
    ...
    public static void main(String[] args){
        ...
        IntStream.range(0, n).forEach(i -> matrix[i] = Arrays.stream(sc.nextLine().strip().split("")).mapToInt(Integer::parseInt).toArray());
